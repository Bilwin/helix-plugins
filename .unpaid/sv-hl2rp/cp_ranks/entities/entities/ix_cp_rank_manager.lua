...
ENT.Base = "base_ai"
ENT.Type = "ai"
ENT.PrintName = "Офицер Civil Protection"
ENT.Category = "[SV] CP"
ENT.Spawnable = true
ENT.AdminOnly = true
ENT.bNoPersist = true
...
function ENT:Initialize()
	if (SERVER) then
		...
		self:SetBodyGroups("01010000020")
		...
		self:SetDisplayName("MPF-OFC #0002")
		self:SetDescription("Выдача рангов для сотрудников Гражданской Обороны")
...
if (SERVER) then
	function ENT:Use(ply)
		if (ply.ixRankManagerCD or 0) > CurTime() then return end
		if !ply:IsCombine() then ply:Notify("Только сотрудники Г.О. могут обращаться ко мне!") ply.ixRankManagerCD = CurTime() + 1 return end
		netstream.Start(ply, 'CP.RankManager.Open', self)
		ply.ixRankManagerCD = CurTime() + 1
	end
else
	...
	netstream.Hook('CP.RankManager.Open', function(ent)
		local panel = vgui.Create("ixCP_RankManager")
		panel:SetEnt(ent)
	end)
end
