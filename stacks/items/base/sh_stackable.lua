...
ITEM.functions.split = {
	...
	OnRun = function(item)
		...
		client:RequestString('Split', 'Please enter how many items you want to split', function(splitStack)
			splitStack = tonumber(splitStack)
			if not isnumber(splitStack) then
				client:Notify('Please enter a number')
				return false
			end

			local cleanSplitStack = math.Round(math.abs(splitStack))
			if cleanSplitStack >= stacks then return false end

			local stackedCount = stacks - cleanSplitStack

			if not character:GetInventory():Add(itemUniqueID, 1, {stacks = cleanSplitStack}) then
